...
void polyExp(int *f, int *g, int bit) {
  if(bit == 0) { g[0] = 1; return ; }
  polyExp(f, g, bit-1);
  int len = 1<<bit, lim = len<<1;
  for(int i = 0; i < lim; ++i) g1[i] = f1[i] = h[i] = 0;
  polyLn(g, h, bit);
  h[0] = (1-h[0]+f[0])%MOD;
  for(int i = 1; i < len; ++i) h[i] = (-h[i]+f[i])%MOD;
  NTT(g, lim, bit+1, 1), NTT(h, lim, bit+1, 1);
  for(int i = 0; i < lim; ++i) g[i] = 1LL*g[i]*h[i]%MOD;
  NTT(g, lim, bit+1, -1);
  for(int i = len; i < lim; ++i) g[i] = 0;
}

void polyPow(int *f, int *g, int bit, int k) {
  int len = 1<<bit, lim = len<<1;
  polyLn(f, h1, bit);
  for(int i = 0; i < len; ++i) h1[i] = 1LL*k*h1[i]%MOD;
  polyExp(h1, g, bit);
}
...
int main() {
  ...
  scanf("%s", s);
  int t = strlen(s);
  for(int i = 0; i < t; ++i) k = (10LL*k+s[i]-'0')%MOD;
