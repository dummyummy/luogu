...
int n;
int f[4*N+5], g[4*N+5], f1[4*N+5], s1[4*N+5], s2[4*N+5], g1[4*N+5], h1[4*N+5];
...
void polyInv(int *f, int *g, int bit) { //f1
...
void polySqrt(int *f, int *g, int bit) { //s1 and s2
    if(bit == 0) {
        g[0] = 1;
        return ;
    }
    ...
    for(int i = 0; i < len; ++i) f1[i] = s1[i] = 0, s2[i] = f[i];
    polyInv(g, s1, bit);
    NTT(s2, lim, bit+1, 1), NTT(s1, lim, bit+1, 1);
    for(int i = 0; i < lim; ++i) s1[i] = 1LL*s1[i]*s2[i]%MOD;
    NTT(s1, lim, bit+1, -1);
    for(int i = 0; i < len; ++i) g[i] = 1LL*(s1[i]+g[i])%MOD*inv2%MOD;
}
...
void polyLn(int *f, int *g, int bit) { //g1
  int len = 1<<bit, lim = len<<1;
...
void polyExp(int *f, int *g, int bit) { //h1
  if(bit == 0) { g[0] = 1; return ; }
  polyExp(f, g, bit-1);
  int len = 1<<bit, lim = len<<1;
  for(int i = 0; i < lim; ++i) g1[i] = f1[i] = h1[i] = 0;
  polyLn(g, h1, bit);
  h1[0] = (1-h1[0]+f[0])%MOD;
  for(int i = 1; i < len; ++i) h1[i] = (-h1[i]+f[i])%MOD;
  NTT(g, lim, bit+1, 1), NTT(h1, lim, bit+1, 1);
  for(int i = 0; i < lim; ++i) g[i] = 1LL*g[i]*h1[i]%MOD;
  NTT(g, lim, bit+1, -1);
  for(int i = len; i < lim; ++i) g[i] = 0;
}
